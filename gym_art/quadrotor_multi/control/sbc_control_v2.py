...
class MellingerControllerV2(object):
    ...
    def step(self, dynamics, dt, observation, goal):
        ...
        if new_acc is not None:
            ...
            acc_des = np.array(new_acc)
        else:
            if self.sbc_last_safe_acc is not None:
                acc_des = np.array(self.sbc_last_safe_acc)
            else:
                acc_des = np.array([0.0, 0.0, 0.0])

        # Question: Why do we need to do this???
        acc_for_control_without_grav = np.array(acc_des)
        acc_for_control_with_grav = acc_des + np.array([0, 0, GRAV])
        ...
        return self.action, acc_for_control_without_grav, 0.0, False
